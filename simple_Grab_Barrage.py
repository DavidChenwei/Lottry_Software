...
# from Dream_Box_BUFF_Cal import cal_buff_box
from utils.Database_operation import create_every_around
...
# from New_Lottery_Judgment_Nocsv import judge_new_lottery
...
# from Searach_list import time_search, id_search
...
def grab_info(room_id: str, time_stamp: str, Rmb_back: str):
    ...
    Rmb_back = float(Rmb_back)
    # Five_mtl_price = float(gl.get_value('Five_mtl_price'))
    Double_mtl_price = float(gl.get_value('Double_mtl_price'))
    room_id = room_id
    time_id = time_stamp
    ...
    try:
        ...
        for i in msg_list:
            ...
            if len(ls) <= 10:
                ...
                ls.append(msg_id)
            # username = '读着倒得字名个这'
            # content = '读着倒得字名个这 赠送 梦幻迷你盒 变幻出 梦幻迷迭香 x1个'
            if msg_type in (7, 33, 37):
                ...
                if username == '企鹅电竞系统提示':
                    ...
                    if '主播发起了抽奖活动，进入企鹅电竞APP马上参与' in content and bln_mention_ext:
                        ...
                        # gl.set_value('this_time_RMB_total', round(this_time_RMB_total, 2))
                ...
                if bln_first_res and username != '':
                    ...
                    if '梦幻盒子' in content or '梦幻迷你盒' in content or '至尊盒子' in content:  # 盒子和普通礼物的弹幕不同，所以要区别
                        ...
                        if bln_DBJ:
                            ...
                            # bln_TimeStamp_ext = time_search(time_stamp_list, time_stamp)  # 查找礼物的时间戳是否已经存在
                            # bln_msgId_ext = id_search(id_list, msg_id)  # 查找礼物的msg_id是否已经存在
                            if bln_TimeStamp_ext and bln_msgId_ext:
                                ...
                                #  writeTotxt(content)  # 记录所有送礼信息已检测是否有误，后期删除
                                # if '梦幻盒子 爆出 BUFF' in content:
                                #     content = content.replace('梦幻盒子 爆出 BUFF', '梦幻盒子 变幻出 ')
                                # if '梦幻迷你盒子 爆出 BUFF' in content:
                                #     content = content.replace('梦幻迷你盒子 爆出 BUFF', '梦幻盒子 变幻出 ')

                                if '爆出 BUFF' in content:
                                    # record = content + " " + msg_id + ' ' + str(otherStyleTime)
                                    # text_create('高级礼物弹幕记录', record)
                                    (current_gift_value, NameGift, account) = caldiamond_buff(content, username,
                                                                                              otherStyleTime,
                                                                                              msg_id)
                                    ...
                                    # NameGift = NameGift.replace(" ", '')
                                    # cal_local_gift_value_diamond = cal_buff_box(NameGift, account, cal_local_gift_value_diamond)
                                    ...
                                    gl.set_value('solo_total_gift_value_diamond',
                                                 local_gift_value_diamond)  # 将礼物钻石总价值存入全局变量
                                    ...
                                    try:
                                        if '梦幻' in lottery_gift_name or lottery_gift_name is None:  #
                                            ...
                                            # print(username + " 送出 ", account, " 个 ", NameGift.replace(" ", ''), " 价值 ",
                                            #       current_gift_value, " 钻石 ")
                                            # print('礼物砖石总价值为: ', total_gift_value_diamond)
                                            ...
                                            gl.set_value('total_gift_value_diamond',
                                                         total_gift_value_diamond)  # 将礼物总价值存入全局变量
                                            gl.set_value('total_rmb_value',
                                                         round(total_rmb_value, 2))  # 将返点后所得RMB存入全局变量
                                            # 获取抽奖的金额以此来计算盈亏
                                    ...
                                    (current_gift_value, NameGift, account) = caldiamond(content, username,
                                                                                         otherStyleTime,
                                                                                         msg_id)
                                    # print(content + '价值 ', current_gift_value, '钻石')
                                    # print(username + " 送出 ", account, " 个 ", NameGift.replace(" ", ''), " 价值 ",
                                    #       current_gift_value, " 钻石 ")
                                    NameGift = NameGift.replace(" ", '')
                                    local_gift_value_diamond += current_gift_value  # 单独计算盒子
                                    ...
                                    if NameGift == '梦幻摩天轮' and '梦幻盒子' in content:
                                        ...
                                        # try:
                                        #     if account == 2:
                                        #         local_money_back += Double_mtl_price
                                        #     elif account == 4:
                                        #         local_money_back += Three_mtl_price - 1000
                                        #     else:
                                        #         local_money_back += Mtl_price
                                        # except Exception as e:
                                        if account >= 2:
                                    ...
                                    final_balance = other_gift_total_diamond - money_give_out + local_total_rmb_value - local_money_back
                                    gl.set_value('this_time_RMB_total', round(final_balance, 2))
                                    # print('其他礼物价值', other_gift_total_diamond)
                                    # print('红包总额', money_give_out)
                                    # print('盒子价值', local_total_rmb_value)
                                    # print('盒子返现', local_money_back)
                                    # print(final_balance)
                                    try:
                                        if '梦幻' in lottery_gift_name or lottery_gift_name is None:
                                            total_gift_value_diamond += current_gift_value  # 累加砖石价值
                        ...
                        if bln_NGJ:
                            ...
                            if bln_TimeStamp_ext and bln_msgId_ext:
                                ...
                                # print(content)
                                # print(gift_name, gift_account)
                                current_gift_value = normal_gift_cal(gift_name, gift_account)  # 计算礼物的钻石价值
                                # print(username + ' 送出 ' + str(gift_account) + " 个 " + gift_name + " 价值 " + str(
                                #     current_gift_value) + " 砖石 ")
                                if gift_name in lottery_gift_name or lottery_gift_name is None:  #
                                    ...
                                    try:
                                        ...
                                        # 将结果存入全局变量
                                        # gl.set_value('RMB_total_value', round(total_rmb_value, 2))
                                        # gl.set_value('RMB_balance', RMB_balance)
                                        # Dict_user 用于保存每轮的用户和钻石
                                        if Dict_user.__contains__(username):
                                            last_account = Dict_user.get(username)
                                            total = current_gift_value + int(last_account)
